...
function M.chain_commands(task_name, commands, module_config, addition_args, previous_job)
    ...
    local job = Job:new({
        ...
        on_start = quickfix_output and vim.schedule_wrap(function()
            print('opening window 1 ' .. command.cmd .. ' ' .. table.concat(args, ' '))
    ...
    if not command.ignore_stdout then
        print('opening window 2 ' .. command.cmd)
        job.stdout:read_start(read_to_quickfix(command.errorformat))
    end
    if not command.ignore_stderr then
        print('opening window 3 ' .. command.cmd)
        job.stderr:read_start(read_to_quickfix(command.errorformat))
    end

    if #commands ~= 1 then
        job:after_success(vim.schedule_wrap(function() M.chain_commands(task_name, vim.list_slice(commands, 2), module_config, addition_args, job) end))
    end
    last_job = job
end

---@return string?
function M.get_current_job_name()
