...
function M.notify(msg, log_level) vim.notify(msg, log_level, { title = 'Tasks' }) end
...
function M.get_module(module_type)
    if module_type == 'auto' then
        for _, name in ipairs(M.get_module_names()) do
        ...
        M.notify('Unable to autodetect module for this working directory', vim.log.levels.ERROR)
        return nil, nil
    end

    local module = require('tasks.module.' .. module_type)
    if not module then
        M.notify('Unable to find a module named ' .. module_type, vim.log.levels.ERROR)
        return nil, nil
    end
