...
local function get_target_names()
    local build_dir = parse_dir(config.cmake.build_dir, config.cmake.build_type)
...
--- TODO reconfig clangd
local function copy_compile_commands()
    local filename = 'compile_commands.json'
    local source = parse_dir(config.default_params.conan.build_dir, config.default_params.conan.build_type) / filename
...
local function install(module_config, _)
    ...
    return {
        ...
        cmd = module_config.conan_cmd,
        args = { 'install', '../..', '-s', 'build_type=' .. module_config.build_type, '-pr:h', 'default', '-pr:b', 'default' },
    }
end
...
local function configure(module_config, _)
    ...
    return {
        ...
        args = {
            ...
            '-D',
            'CMAKE_TOOLCHAIN_FILE=' .. 'build/' .. module_config.build_type .. '/generators/conan_toolchain.cmake',
        },
...
local function build(module_config, _)
    ...
    return {
        cmd = module_config.cmake_cmd,
        args = { '--build', build_dir.filename, '-j' },
    }
end
...
M.condition = function() return Path:new('conanfile.py'):exists() end
